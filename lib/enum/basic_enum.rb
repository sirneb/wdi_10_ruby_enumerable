...
class MyEnumeration
  ...
  def contains_a_word_longer_than?(min_length)
    collection.any? {|c| c.length > min_length}
  end
  ...
  def element_frequency_count
    collection.each_with_object(Hash.new(0)) {|c, obj| obj[c] += 1}
  end
  ...
  def sum_of_experiences
    collection.reduce(0) {|memo, c| memo += c[:years_experience]}
  end
  ...
  def longest_element_name_using_inject
    collection.reduce do |memo, c|
      if c[:name].length > memo[:name].length
        c
      else
        memo
      end
  ...
  def most_experienced_element
    collection.max_by {|c| c[:years_experience]}
  end
  ...
  def elements_with_longest_and_shortest_names
    collection.minmax_by {|c| c[:name].length}
  end
  ...
  def elements_sorted_by_experience
    collection.sort_by {|c| c[:years_experience]}
  end
